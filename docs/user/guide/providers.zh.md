...
同时提供 OAuth 的提供方——其中包括 Anthropic——会在密钥字段旁显示**登录**按钮。点击后会打开一个对话框，转发该登录流程自身的提示：通常是一个需要在浏览器中打开并完成的链接，也包含（Anthropic 自身的流程同样）一个用于粘贴授权码或最终跳转 URL 的输入框，供运行 dsh 的机器与浏览器不在同一台设备时使用。流程报告成功后凭据即被存储，卡片无需刷新页面即可感知。
